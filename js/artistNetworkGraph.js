class ArtistNetworkGraph {
  constructor(container, options = {}) {
    ...
    this.instructionsDismissed = false;
  ...
  init() {
    ...
    // Creates tooltip and stores it in state
    ...
    // Disables fullpage.js scrolling events
    ...
    // Bind SVG background clicks
    d3.select("#overlay").on("click", () => {
  ...
  //! UI interaction and display handling logic
  /**
   * init method helper]
   * Creates and configures an SVG container for rendering the artist network graph.
   */
  createSVGContainer() {
  ...
  /**
   * [init method helper]
   * Initializes zoom configuration.
   */
  setupZoom(svg, graphGroup) {
    const zoom = d3.zoom()
      ...
      .on("zoom", event => {
        ...
        this.state.currentZoomScale = event.transform.k;
      });
    svg.call(zoom);
  ...
  /**
   * Sets up all user interface controls and event listeners for the artist network.
   */
  setupUIControls() {
      ...
      .on("keydown", function(event) {
        ...
        // Arrow Up: move selection up
          ...
          if (currentSuggestionIndex === -1) {
              currentSuggestionIndex = numSuggestions - 1;
          } else {
  ...
  //! Primary data loading logic
  /**
   * Updates the internal state for parameterized values and stores maximum collaborators
   * node sizing scale, and renders network graph with top-k filters.
   */
  initializeGraphDisplay(nodes, links, artistInfo) {
  ...
  /**
   * Loads artist display photo datasets.
   */
  loadArtistImages() {
  ...
  /**
   * Loads and processes network data and artist information (if cached) for a given year.
   */
  loadData(year) {
    ...
    if (this.dataCache[year]) {
        ...
        const { nodes, links, artistInfo } = this.dataCache[year];
        this.initializeGraphDisplay(nodes, links, artistInfo);
    ...
    Promise.all([
      ...
      const { nodes, links } = this.processData(networkData, artistInfo);

      // Cache processed data
      this.dataCache[year] = { nodes, links, artistInfo };
      this.initializeGraphDisplay(nodes, links, artistInfo);
      this.restoreHistoryForYear(year);
  ...
  /**
  * Processes the raw network CSV data and artist-level JSON data into structured
  * nodes and links suitable for rendering the artist collaboration graph.
  */
  processData(networkData, artistInfo) {
    const nodesMap = {};

    // Data collected from the artist-level information JSONs
    Object.entries(artistInfo).forEach(([artistId, info]) => {
        nodesMap[artistId] = {
            ...
            degree: 0 // placeholder for lower population
        };
    ...
    const links = networkData
      ...
      .map(d => {
          ...
          // Only increment degrees if nodes exist
  ...
  //! Graph rendering and main visualization logic
  /**
   * Creates and renders artist network graph via force simulation with finite tick counts
   * with nodes, links, and associated interactions
   */
  createGraph(nodes, links) {
    ...
    this.state.overlayElements = graphGroup.append("g")
        ...
        .enter().append("circle")
        .attr("class", "overlay")
        .attr("r", d => this.state.totalStreamRadiusScale(d.total_streams) + 4) // Slightly oversized to show
        .attr("cx", d => d.x)
        .attr("cy", d => d.y);
  ...
  /**
   * Computes the bounding window with extremas to fit the highlighted network of
   * a selected artist so that it is not hidden behind the info panel
   */
  ...
  //! Node highlighting and focused data filtering applications
  /**
   *  Highlights the selected artist node and its direct collaborators (neighbors).
   * @param {*} selected - the user selected node
   */
  ...
  /**
   * Identifies and annotates special artists fitting the criteria
   * - are solo artits and have the most streams
   * - are collab artists and have the most streams
   * - are collab artists and have the most collaborations
   */
  labelSpecialArtists() {
    ...
    const placeTooltipForNode = async (node) => {
      ...
      if (!nodeSel.empty()) {
        nodeSel.on("mouseover", (event, d) => {
          ...
          //! Enforce the same font sizes you use for regular nodes
  ...
  /**
   * Filters and visually highlights the top-K ranked artists on the network graph.
   * Top-k is a synonym for most streamed artist for some integer boundary k.
   */
  applyTopKFilter() {
  ...
  //! Instruction display updating logic
  /**
   * Resets the full network graph to its default state.
   */
  resetVisualization() {
  ...
  /**
   * Reveals instruciton panel on artist network display, showing how a user can interact with the display
   */
  ...
  /**
   * Helper function for showInstructionPanel to generate the green gradient bar legend.
   */
  createInstructionLegend(container) {
    // Dimensions for the legend rectangle SVG.
    ...
    // Reverse green scale.
    // Darker = fewer collaborators
    // Lighter = more collaborators
    legendGradient.append("stop")
      .attr("offset", "0%")
      .attr("stop-color", this.reverseGreenScale(0));

    legendGradient.append("stop")
      .attr("offset", "100%")
      .attr("stop-color", this.reverseGreenScale(1));

    // Group that holds the rectangle and labels,
  ...
  /**
   * Displays detailed information about the selected into the info panel on the artist view.
   * @param {Object} artist The selected artist from an artist search event.
   */
  ...
  /**
   * Shows the side icon which toggles instruction and artist views.
   */
  ...
  /**
   * Clears and hides the info panel.
   */
  clearInfoPanel() {
  ...
  /**
   * Builds and renders a sortable table of the artist's songs inside the info panel.
   * @param {Object} artist The selected artist from an artist search event.
   */
  ...
  /**
   * Helper function to minimize detailed informaiton pane and trigger a toggle event
   * for that view.
   */
  minimizeInfo() {
    d3.select("#info-panel").style("display", "none");
  ...
  /**
   * Resets the instruction toggle icon and related UI state when switching datasets (years).
   */
  ...
  //! Search history (log history) handling
  /**
   * Restores cached search history/logged content assuming the year contains information.
   * @param {*} year the year which the user has selected.
   */
  ...
  /**
   * Pushes a search event to the history cache.
   */
  ...
  /**
   * Artist info pane back button logic to help navigate through search history/logs.
   */
  goBack() {
  ...
  /**
   * Artist info pane forward button logic to help navigate through search history/logs.
   */
  goForward() {
      const historyObj = this.state.historyByYear[this.currentYear];
  ...
  /**
   * Updates the enabled/disabled state of the back and forward navigation buttons
   * based on the current position within the artist selection history.
   */
  ...
  /**
   * Searches for an artist by name, highlights the artist on the graph, and updates the info panel.
   */
...
//! HELPER FUNCTIONS
// Helper function to highlight a suggestion in the fuse.js suggestions
