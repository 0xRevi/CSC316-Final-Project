class BarChart {
    ...
    async loadData() {
        ...
        raw = raw.filter(
            (d) => d.parent_genres && d.parent_genres.toLowerCase() !== "nan"
        );
        return raw;
    }
    ...
    processData(data) {
        ...
        for (const [year, genreMap] of nested.entries()) {
            ...
            for (const [genre, rows] of genreMap.entries()) {
                const soloCount = rows.filter(
                    (r) => r.song_type === "Solo"
                ).length;
                const collabCount = rows.filter(
                    (r) => r.song_type === "Collaboration"
                ).length;
    ...
    render() {
        this.loadData().then((rawData) => {
            ...
            const colorScale = d3
                ...
                .domain(genreArray)
                .range(
                    d3.schemeCategory10.concat(d3.schemeSet3, d3.schemeDark2)
                );
            ...
            yearGroup.each(function (d) {
                ...
                const gSelection = d3
                    .select(this)
                    .selectAll(".genreStack")
                    .data(groupData)
                    .join("g")
                    .attr("class", "genreStack")
                    .attr(
                        "transform",
                        (item) => `translate(${x1(item.parent_genres)}, 0)`
                    );
